...
# Пояснение к статусу колеса в тексте уведомления. Свежая находка доходит
# сюда только со статусом active — уведомление требует подтверждённого
# колеса (см. betboom.process_candidate_wheel). Прочие статусы остались
# ради записей, сохранённых до появления белого списка: их дошлёт
# retries.retry_failed_notifications, и статус там может быть любым.
STATUS_NOTES = {
    "active": "колесо активно",
    "soon": "розыгрыш ещё не начался",
    "unknown": "проверить не удалось — статус смотри сам",
    "missing": "страницы колеса не существует",
}
# В сообщении о нескольких ссылках пояснения короче.
MULTI_STATUS_NOTES = {
    "active": "колесо активно",
    "soon": "розыгрыш ещё не начался",
    "expired": "уже завершилось",
    "missing": "страницы нет",
...
def send_multi_telegram_notification(
    ...
    прошлого поста, рядом с актуальной ссылкой. Прекчек отсеивает хвосты,
    но одновременно живых колёс в посте бывает несколько (стример
    запускает розыгрыш на нескольких адресах), поэтому вместо N отдельных
    «Новая ссылка» шлём одно сообщение со списком.
    """
    if not notifications_enabled():
        return False
    first = entries[0]
